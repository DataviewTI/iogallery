new IOService(
  ...
  function(self) {
            ...
            render: function(data, type, row) {
              ...
              data.forEach(function(c) {
                ...
                if (
                  c.parent != "" &&
                  c.parent != null &&
                  !cats.includes(c.maincategory.category)
                )
    ...
    self.dimensions_dt = $("#__dimensions")
      .DataTable({
        paging: false,
        info: false,
        ordering: false,
        initComplete: function() {
          let api = this.api();
          api.row.add(["thumb", "240", "180", null]).draw(true);
        ...
        fields: {
          __cat_subcats: {
            validators: {
              callback: {
                message:
                  "A galeria deve ter no mínimo uma categoria vinculada!",
                  ...
                  if (video.source != null) {
                    ...
                    prom
                      .then((resolve) => {
                        ...
                        if (self.loadedVideo == null) self.loadedVideo = video;
    ...
    //instancia da categoria
    new Category({ parentService: self });
  }
); //the end ??
...
function addDimension(p) {
  p.self.dimensions_dt.row
    .add([p.prefixo, p.largura, p.altura, null])
    .draw(true);
      ...
      .attrchange(function(attrName) {
        if (attrName == "class") {
          if ($(this).hasClass("active")) {
            self.loadedVideo.thumbnail = {
              pos: $(this).attr("data-pos"),
              url: $(self.dz.files[0].previewTemplate)
...
function addVideoToList(video, self) {
  ...
  var $newVideo = $(
    `
    <div class="video-box col-2" data-video-data='` +
  ...
  $newVideo.find(".dz-edit").on("click", function() {
    ...
    self.fv[3].revalidateField("video_url").then(function() {
      ...
      $(".video-thumb").removeClass("active");
      $('.video-thumb[data-pos="' + video_data.thumbnail.pos + '"]').addClass(
        "active"
      );
